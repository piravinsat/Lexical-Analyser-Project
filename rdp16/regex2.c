/*******************************************************************************
*
* Parser generated by RDP on Oct 20 2013 22:58:21 from regex2.bnf
*
*******************************************************************************/
#include <time.h>
#include "regex2.h"
...
char
  ...
  *rdp_outputfilename = "regex2.out";         /* output file name */
...
/* Set declarations */
  ...
  set_ reg2_first = SET_NULL;
  set_ reg2_stop = SET_NULL;

/* Initialise sets */

static void rdp_set_initialise(void)
{
  set_assign_list(&alt_stop, SCAN_P_EOF,SET_END);
  set_assign_list(&reg_first, SCAN_P_ID, RDP_T_35 /* # */, SET_END);
  set_assign_list(&reg_stop, SCAN_P_EOF,SET_END);
  set_assign_list(&reg2_first, SCAN_P_ID, RDP_T_35 /* # */, SET_END);
  set_assign_list(&reg2_stop, SCAN_P_EOF,SET_END);
}

/* Parser forward declarations and macros */
static void alt(void);
void reg(void);
static void reg2(void);
...
{
  {
    if (scan_test(NULL, SCAN_P_ID, NULL))
    {
      scan_test(NULL, SCAN_P_ID, &reg2_stop);
      scan_();
    }
    else
    if (scan_test(NULL, RDP_T_35 /* # */, NULL))
    {
      scan_test(NULL, RDP_T_35 /* # */, &reg2_stop);
      scan_();
    }
    else
      scan_test_set(NULL, &reg2_first, &reg2_stop)    ;
    scan_test_set(NULL, &reg2_stop, &reg2_stop);
   }
}
