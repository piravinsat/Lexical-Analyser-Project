/*******************************************************************************
*
* Parser generated by RDP on Oct 07 2013 19:00:20 from expr1.bnf
*
*******************************************************************************/
#include <time.h>
#include "expr1.h"
...
char
  ...
  *rdp_outputfilename = "expr1.out";         /* output file name */
...
static void rdp_load_keywords(void)
{
  ...
  scan_load_keyword("b", NULL, RDP_T_b, SCAN_P_IGNORE);
}
...
/* Set declarations */
  ...
  set_ T_first = SET_NULL;
  set_ T_stop = SET_NULL;
  set_ X_stop = SET_NULL;
  set_ Y_stop = SET_NULL;
  set_ rdp_E_0_first = SET_NULL;
  set_ rdp_S_0_first = SET_NULL;

/* Initialise sets */

static void rdp_set_initialise(void)
{
  set_assign_list(&E_first, RDP_T_a, RDP_T_b, SET_END);
  set_assign_list(&E_stop, SCAN_P_EOF, RDP_T_43 /* + */,SET_END);
  set_assign_list(&S_first, RDP_T_a, RDP_T_b, SET_END);
  set_assign_list(&S_stop, SCAN_P_EOF,SET_END);
  set_assign_list(&T_first, RDP_T_a, RDP_T_b, SET_END);
  ...
  set_assign_list(&rdp_E_0_first, RDP_T_a, RDP_T_b, SET_END);
  set_assign_list(&rdp_S_0_first, RDP_T_a, RDP_T_b, SET_END);
}
...
{
  {
    ...
    {
      scan_test(NULL, RDP_T_a, &T_stop);
      scan_();
    }
    else
    if (scan_test(NULL, RDP_T_b, NULL))
    {
      scan_test(NULL, RDP_T_b, &T_stop);
      scan_();
    }
    else
      scan_test_set(NULL, &T_first, &T_stop)    ;
